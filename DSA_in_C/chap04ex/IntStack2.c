...
int Initialize(IntStack *s, int max)
{
    s->ptrA = 0;
    s->ptrB = max - 1;
    if ((s->stk = calloc(s->max, sizeof(int))) == NULL) {
        s->max = 0;
        return -1;
    }
    s->max = max;
...
{
    switch (sw) {
        case StackA : return s->ptrA;
        case StackB : return s->max - s->ptrB - 1;
    }
}
...
{
    switch (sw) {
        case StackA : return s->ptrA <= 0;
        case StackB : return s->ptrB >= s->max - 1;
    }
}
